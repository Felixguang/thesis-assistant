...
def detect_direction(title: str) -> str:
    """基于 879 条真实正选统计的方向识别（4 大类 + 其他）。

    判别原则（依据数据治理经验）：
      - 研究主语优先于找开发者的关键词指纹
      - "翻译/译本/详庅" 为主语表主以翻译类
      - "文化差异/跨文化/品牌文化" 为主语表跨文化类
      - "话语/语用/隐喻/批评话语" 为主语表话语分析
      - "教学/习得/学习/教学法/教材评估" 为主语表商务英语习得
    """
    ...
    for direction, patterns in [
        ...
        ("翻译", ["翻译", "translation", "英译", "汉译", "译本",
                   "字幕翻译", "归化", "异化", "外宣翻译", "译介", "创译",
                   "目的论", "译配", "可译性", "翻译策略", "翻译美学",
                   "翻译策略研究", "口译", "字幕", "英汉互译", "汉英互译",
                   "误译", "译本", "译文"]),
        # 跨文化类：以文化差异/跨文化为主语
        ("跨文化", ["跨文化", "cross-cultural", "高低语境", "文化维度",
                     ...
                     "文化冲突", "文化差异", "文化空缺", "文化负载",
                     "文化传承", "文化内涵", "文化特征", "文化根源",
                     "文化塑造", "文化导入", "跨文化交际", "跨文化沟通",
                     "跨文化传播", "跨文化叙事", "文化误读", "文化偶像",
                     "文化象征"]),
        # 话语分析类：以话语/语用/隐含等方法论为主语
        ("话语分析", ["话语分析", "多模态", "视觉语法", "批评话语",
                       "批评性话语", "discourse", "语篇", "系统功能",
                       "人际意义", "互文性", "多模态隐喻", "语用",
                       "pragmatic", "礼貌原则", "合作原则", "面子理论",
                       "言语行为", "元话语", "模糊限制语", "话语策略",
                       "修辞", "劝说", "说服", "概念隐喻", "符号学",
                       "评价理论", "态度系统", "语域", "语用学",
                       "中式英语", "人际功能", "及物性", "人际关系管理",
                       "隐喻策略", "隐喻构建", "隐喻意义", "语用特点",
                       "语用功能", "语用失误", "语用模糊", "语用补偿",
                       "概念功能", "语篇分析", "话语建构", "形象建构",
                       "话语标记", "虚拟语气", "模糊语", "顺应论",
                       "顺应理论"]),
        # 商务英语习得类：以教学/学习/习得为主语
        ("商务英语习得", ["习得", "二语", "外语教学", "词汇习得",
                       "词汇学习", "词汇记忆", "教学法", "课程", "教材",
                       "教材评估", "批改", "TPACK", "建构主义", "构建主义",
                       "学习动机", "学习兴趣", "输入假设", "Krashen",
                       "POA", "产出导向法", "写作能力", "学习策略",
                       "教学策略", "教学案例", "翻转课堂", "教学模式",
                       "口语教学", "听力教学", "阅读教学", "中介语",
                       "偏误", "克拉申", "情感过滤", "监控假说",
                       "教学反馈", "同伴互动", "形成性评价", "词汇记忆",
                       "学习动机", "学习焦虑", "多元智能", "联通主义",
                       "认知负荷", "词典app", "词典学", "学业成绩",
                       "交际意愿"]),
    ]:
    ...
    if not scores:
        return "其他"
    # 优先级调整：当多个方向都有命中时，根据强信号词判断
    # 1. "翻译" 强于 "话语分析"（多模态话语分析视角下的游戏翻译）
    if scores.get("翻译", 0) > 0 and scores.get("话语分析", 0) > 0:
        if any(p in title for p in ["翻译", "译本", "英译", "汉译", "详庅", "译介", "翻译策略", "口译"]):
            return "翻译"
    # 2. "跨文化" 强于 "话语分析"（跨文化语用学下的文化对比）
    if scores.get("跨文化", 0) > 0 and scores.get("话语分析", 0) > 0:
        if "跨文化" in title or "文化对比" in title or "文化差异" in title:
            return "跨文化"
    # 3. "商务英语习得" 强于 "其他"（交际意愿、学业成绩、词典app 等）
    if scores.get("商务英语习得", 0) == 0:
        if any(p in title for p in ["词典app", "词典学", "学业成绩", "学习动机",
                                     "学习兴趣", "学习策略", "词汇记忆", "词汇习得",
                                     "词汇学习", "学习焦虑", "输入假设", "习得策略"]):
            return "商务英语习得"
...
def get_library_insights() -> dict:
    ...
    dir_counter = Counter()
    for t in topics:
        dir_counter[t.get("direction", "其他")] += 1

    # 各届分布
    year_counter = Counter()
    for t in topics:
        year_counter[t.get("year", "?")] += 1
    ...
    return {
        "total": len(topics),
        "direction_stats": dict(dir_counter.most_common()),
        "year_stats": dict(sorted(year_counter.items())),
